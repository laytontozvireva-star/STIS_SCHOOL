...
const EMPTY_FORM = { name: "", email: "", password: "", role: "teacher", subject: "", department: "" };
...
const ManageAccounts = () => {
  ...
  const submit = async (event) => {
    ...
    try { const result = await createPrivilegedAccount(form); setForm(EMPTY_FORM); setMessage(result.message); }
    catch (error) { setMessage(error.message || "Could not create this account."); }
    finally { setSaving(false); }
  };

  return <div className="mx-auto max-w-3xl">
    <div className="mb-8"><h1 className="font-heading text-2xl font-bold text-textPrimary">Create Staff Account</h1><p className="mt-2 font-body text-sm text-textSecondary">Teachers receive an email invitation and set their own password. Parent and administrator accounts use a temporary password.</p></div>
    <form onSubmit={submit} className="rounded-2xl border border-border bg-surface p-6 shadow-sm sm:p-8">
      <div className="flex items-center gap-3"><span className="rounded-xl bg-primary/10 p-3 text-primary"><ShieldPlus className="h-6 w-6" /></span><div><h2 className="font-heading text-lg font-semibold text-textPrimary">Account details</h2><p className="font-body text-sm text-textSecondary">{isTeacher ? "An invitation link will be emailed to the teacher." : "Give the account owner a secure temporary password."}</p></div></div>
      <div className="mt-6 grid gap-5 sm:grid-cols-2"><Field label="Full name" name="name" value={form.name} onChange={setField} required /><Field label="Email" name="email" type="email" value={form.email} onChange={setField} required /><label className="block"><span className="font-body text-sm font-medium text-textPrimary">Role</span><select name="role" value={form.role} onChange={setField} className="mt-1.5 w-full rounded-xl border border-border bg-white px-3 py-2.5 font-body text-sm outline-none focus:ring-2 focus:ring-primary"><option value="teacher">Teacher</option><option value="parent">Parent</option><option value="admin">Administrator</option></select></label>{!isTeacher && <Field label="Temporary password" name="password" type="password" value={form.password} onChange={setField} minLength="8" required />}{isTeacher && <><Field label="Subject (optional)" name="subject" value={form.subject} onChange={setField} /><Field label="Department (optional)" name="department" value={form.department} onChange={setField} /></>}</div>
      <button disabled={saving} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-primary px-5 py-3 font-body text-sm font-semibold text-white hover:bg-primaryDark disabled:opacity-60"><UserPlus className="h-4 w-4" />{saving ? "Creating..." : isTeacher ? "Send teacher invitation" : "Create account"}</button>{message && <p className="mt-4 font-body text-sm text-textSecondary" role="status">{message}</p>}
    </form>
  </div>;
};
